...
`;
      ...
      case "cat":{
        ...
        try{
          if(state.phase >= 5 && path === "/home/player/workbench/ohlendorf/ticket_net.txt"){
...
const outText = (extra + open).trim();
      ...
      case "cp":{
        ...
        try{
          ...
          if(state.phase >= 5 && src === "/real_life/ars_recycling/docs/abholplan_2026.csv" && dst.startsWith("/home/player/workbench/ars/")){
            state.jobArc = state.jobArc || { active:true, stage:0, quests:{ snackmaster:false, ars:false, ohlendorf:false, berndt:false } };
            state.jobArc.active = true;
            state.jobArc.quests = state.jobArc.quests || {};
          ...
          if(state.phase >= 5 && src === "/real_life/ohlendorf_technik/ticket_net.txt" && dst === "/home/player/workbench/ohlendorf/ticket_net.txt"){
...
Wichtig: Nach dem Kopieren → logwipe, sonst bleiben Spuren.` };
  ...
  function runLine(line){
    ...
    // --- TEST-CHEAT ---
    // Eingabe: 1337
    // Springt ans Ende von Phase 4 (Sidequest gilt als erledigt) und teleportiert ins Sekretariat.
    if(trimmed === "1337"){
      // Phasen 1–3 als erledigt markieren
      ...
      // Zeugnis-Arc so setzen, dass nur noch das finale Zeugnis fehlt
      state.flags.report_given = true;
      state.flags.report_followup = true;
      state.flags.report_final = false;
      ...
      // Winkelmann-Sidequest als abgeschlossen
      state.sidequest = state.sidequest || {};
      state.sidequest.unlocked = true;
      state.sidequest.stage = Math.max(state.sidequest.stage||0, 6);
      state.sidequest.badge = true;
      state.sidequest.parts = Object.assign({ lens:true, coil:true, ups:true }, state.sidequest.parts||{});
      state.sidequest.net = Object.assign({ blueprint:true, shield:true }, state.sidequest.net||{});
      state.sidequest.traces = Object.assign({ gym:false, igs:false }, state.sidequest.traces||{});
      state.sidequest.traceMeter = Object.assign({ gym:0, igs:0 }, state.sidequest.traceMeter||{});
      state.sidequest.alarm = Object.assign({ gym:false, igs:false }, state.sidequest.alarm||{});

      // Teleport ins Sekretariat
      state.cwd = "/school/sekretariat";

      // Zeugnis-Datei auf "online" drehen
      try{
        const z = getNode("/school/sekretariat/zeugnis.txt");
        if(z && z.type==="file"){
          z.content = `ZEUGNIS-DRUCK (Status):\n✅ Online — Dienste wieder verfügbar\n\nHinweis:\nSprich im Sekretariat: talk harries  /  talk pietsch`;
        }
      }catch(e){}

      // Rewards (nur, falls noch nicht da)
      try{ award("badge_tutorial"); }catch(e){}
      try{ award("badge_patch"); }catch(e){}
      try{ award("badge_boss"); }catch(e){}
      try{ award("badge_mentor"); }catch(e){}
      ...
      renderRewards();
      renderPhasePill();
      row("🧪 CHEATCODE ACTIVATED: Du wurdest ins Sekretariat gespawnt. Hol jetzt das finale Zeugnis: talk harries / talk pietsch", "ok");
      return;
    }
