...
function allowedCommands(){
  ...
  function grepTrigger(pattern, outText){
    if(state.phase >= 2 && pattern === "FRAG1_TOKEN" && outText.includes("FRAG1_TOKEN=")){
  ...
  function runScript(path, argv){
    ...
    const must = `echo "PATCH_APPLIED"`;
    if(!content.includes(must)){
      return { ok:true, out:`Patchlord lacht: "Bro, wo ist ${must} ?"
(du musst die Zeile rein-echo-en)` };
    }
...
→ Geh ins Sekretariat und hol dein Zeugnis ab: talk harries / talk pietsch` };
  ...
  function cmdImpl(line, stdin=null){
    ...
    if(red){
      ...
      if(target === "/home/player/workbench/patchlord.sh" && t.includes('echo "PATCH_APPLIED"')){
...
const outText = (extra + open).trim();
      ...
      case "ps":{
        ...
        const procs = (state.processes||[]).map(p=>`${p.pid}\t${p.name}`);
      ...
      case "top":{
        ...
        for(const p of procs){
          lines.push(`${String(p.pid).padEnd(4)} ${String(p.cpu).padEnd(5)} ${String(p.mem).padEnd(6)} ${p.name}`);
        }
        return { ok:true, out: lines.join("\n") };
      }
      ...
      case "mentor_clear":{
        ...
        award("badge_mentor");
        row("🎉 Mentor-Run clear! Du hast 3/3 geholfen — und keiner hat sich dabei komplett verloren.", "ok");
